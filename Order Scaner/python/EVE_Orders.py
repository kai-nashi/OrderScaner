    ...
    def run(self):
        ...
        if self.charID != None:
            ...
            if self.orderType != None:
                self.orders = _Orders_LoadFromFile(self.path, self.charID, self.orderType)
            else:
                self.orders = _Orders_LoadFromFile(self.path, self.charID)
...
class ordersLoader_FromAPI(threading.Thread):
    ...
    def run(self):
            ...
            # load orders
        if self.orderType != None:
            self.orders = _Orders_LoadFromAPI(self.key, self.vc, self.charID, self.orderType)
        else:
            self.orders = _Orders_LoadFromAPI(self.key, self.vc, self.charID)
...
class ordersScaner_Row(threading.Thread):
    ...
    def run(self):
        ...
        if self.row < len(self.orders):
            ...
            crestMarket._Order_Update(order)
            self._Orders_UpdateCheckDate(order)
        
    def _Orders_UpdateCheckDate(self, order):
...
def _Orders_LoadFromFile(path, charID, bid = None):
    ...
    orders = _Orders_UpdateItemName(orders)
    
    print(orders)
    return orders
    
def _Orders_LoadFromAPI(key, vc, charID, orderType = None):
    ...
    orders = _Orders_UpdateLocation(orders)
    orders = _Orders_UpdateItemName(orders)

    print(orders)
    return orders
    
def _Orders_UpdateLocation(orders):
...
def _Orders_UpdateItemName(orders):
